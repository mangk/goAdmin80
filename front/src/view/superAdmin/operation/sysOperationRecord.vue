    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="日期" width="180">
          <template #default="scope">{{ formatDate(scope.row.ct) }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="请求方法" prop="method" width="120">
          <template #default="scope">
            <el-tag
                :type="scope.row.method == 'POST' ? 'success' : scope.row.method == 'GET'? 'default' : scope.row.method == 'DELETE' ? 'error' : 'warning'">
              {{ scope.row.method }}
            </el-tag>
          </template>
        </el-table-column>
...
<script setup>
import {deleteSysOperationRecordByIds, getSysOperationRecordList} from '@/api/sysOperationRecord' // 此处请自行替换地址
...
const deleteSysOperationRecordFunc = async (row) => {
  row.visible = false
  const res = await deleteSysOperationRecordByIds({ids: [row.id]})
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
